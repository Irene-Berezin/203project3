...
def good(gst, d, v, w, graph, goal_dist): 
    return (w not in gst.adj[v] and v not in gst.adj[w]
            and graph.edges[v, w, 0]['length'] > 0
            and d + graph.edges[v, w, 0]['length'] <= 1.1 * goal_dist) # add distance to prev distance and make sure less than goal dist
...
def find_route(start, goal_dist, graph):
    ...
    graph.add_edge(start, start, length = 0)
...
def shade_given_time(k, n):
    ...
    result_list = []
    for x in col:
        result_list.append(int(x * 255))
    tup = tuple(result_list)
